...
public class Doctor extends User {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    ...
                    LocalDate apptDate = LocalDate.parse(date, dateFormatter);
                    LocalTime apptTime = LocalTime.parse(time, timeFormatter);
                    
                    // Add accepted appointments for the logged-in doctor
                    if (status.equalsIgnoreCase("Confirmed") && doctorID.equals(this.hospitalID)) {
                        acceptedAppointments.add(new Appointment(doctorID, doctName, patientID, apptDate, apptTime, status));
                    }
                } catch (Exception e) {
                    // Handle any parsing errors (DateTimeParseException, etc.)
                    //System.out.println("Error parsing date or time for appointment: " + e.getMessage());
                	continue;
                }
            }
        } catch (IOException e) {
    ...
    public void recordAppointmentOutcome() {
        ...
        // Check if there are any accepted appointments
        if (acceptedAppointments.isEmpty()) {
            System.out.println("No confirmed appointments found for Doctor ID: " + this.getHospitalID() + "\n");
            return;
        }
